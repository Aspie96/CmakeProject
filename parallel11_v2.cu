...
void kernel1a(const stbi_uc *restrict img, int width, int height, size_t result_pitc, size_t img_pitch, int n, int nblock, unsigned short *restrict result) {
	...
	extern __shared__ stbi_uc tile1[];
	z = blockIdx.x;
	j = blockIdx.z * blockDim.z + threadIdx.z;
	for(b = 0; b < nblock; b++) {
		i = (blockIdx.y * nblock + b) * blockDim.y + threadIdx.y;
		tile1[threadIdx.z * (n - 1 + nblock * blockDim.y) + (n >> 1) + threadIdx.y + blockDim.y * b] = img[(j * img_pitch + i * 3) + z];
	}
	if(threadIdx.y < n - 1) {
		...
		tile1[threadIdx.z * (n - 1 + nblock * blockDim.y) + threadIdx.y + aux2] = (0 <= aux && aux < width) ? img[(j * img_pitch + aux * 3) + z] : 0;
	}
	if(blockDim.y <= 32) {
		f = threadIdx.y;
	} else {
	...
	for(b = 0; b < nblock; b++) {
		...
		if(i < width && j < height) {
			c = 0;
			for(k = 0; k < n >> 1; k++) {
				c += filter1_d[k] * (tile1[threadIdx.z * (n - 1 + nblock * blockDim.y) + b * blockDim.y + threadIdx.y + k] + tile1[threadIdx.z * (n - 1 + NBLOCK * blockDim.y) + b * blockDim.y + threadIdx.y + n - 1 - k]);
			}
			c += filter1_d[k] * tile1[threadIdx.z * (n - 1 + nblock * blockDim.y) + b * blockDim.y + threadIdx.y + k];
...
void kernel1b(unsigned short *img, int width, int height, size_t result_pitc, size_t img_pitch, int n, int nblock, unsigned short *restrict result) {
	...
	for(b = 0; b < nblock; b++) {
		...
		if(i < width && j < height) {
			c = 0;
			for(k = 0; k < n >> 1; k++) {
				c += filter2_d[k] * (tile[threadIdx.z * (n - 1 + nblock * blockDim.y) + b * blockDim.y + threadIdx.y + k] + tile[threadIdx.z * (n - 1 + NBLOCK * blockDim.y) + b * blockDim.y + threadIdx.y + n - 1 - k]);
			}
			c += filter2_d[k] * tile[threadIdx.z * (n - 1 + nblock * blockDim.y) + b * blockDim.y + threadIdx.y + k];
...
void kernel2a(unsigned short *img, int width, int height, size_t result_pitc, size_t img_pitch, int n, int nblock, unsigned short *restrict result) {
	...
	for(b = 0; b < nblock; b++) {
		...
		if(i < width && j < height) {
			c = 0;
			for(k = 0; k < n >> 1; k++) {
				c += filter2_d[k] * (tile[(threadIdx.z + k + blockDim.z * b) * blockDim.y + threadIdx.y] + tile[(threadIdx.z + n - 1 - k + blockDim.z * b) * blockDim.y + threadIdx.y]);
			}
			c += filter2_d[k] * tile[(threadIdx.z + k + blockDim.z * b) * blockDim.y + threadIdx.y];
			result[(z * height + j) * result_pitc + i] = APPROX_DIVIDE2(c, n - 1);
		}
	}
}
...
void kernel2b(const unsigned short *restrict img, int width, int height, size_t result_pitc, size_t img_pitch, int n, int nblock, stbi_uc *restrict result) {
	...
	for(b = 0; b < nblock; b++) {
		...
		if(i < width && j < height) {
			c = 0;
			for(k = 0; k < n >> 1; k++) {
				c += filter1_d[k] * (tile[(threadIdx.z + k + blockDim.z * b) * blockDim.y + threadIdx.y] + tile[(threadIdx.z + n - 1 - k + blockDim.z * b) * blockDim.y + threadIdx.y]);
			}
			c += filter1_d[k] * tile[(threadIdx.z + k + blockDim.z * b) * blockDim.y + threadIdx.y];
...
void blur(int n, int width, int height, stbi_uc *restrict img) {
	...
	kernel1a << <blocks1, threadsPerBlock1, sizeof(stbi_uc) *(threadsH * nBlocksH + 16) >> > (img_d, width, height, aux1_pitch, img_pitch / sizeof(stbi_uc), n_init, nBlocksH, aux1_d);
	for(i = n_init; i < (n - 1); i += 16) {
		kernel2a << <blocks2, threadsPerBlock2, sizeof(unsigned short) *(48 * (32 * nBlocksV + 16)) >> > (aux1_d, width, height, aux2_pitch, aux1_pitch, 17, nBlocksV, aux2_d);
		kernel1b << <blocks1, threadsPerBlock1, sizeof(unsigned short) *(threadsH * nBlocksH + 16) >> > (aux2_d, width, height, aux1_pitch, aux2_pitch, 17, nBlocksH, aux1_d);
	}
	kernel2b << <blocks2, threadsPerBlock2, sizeof(unsigned short) *((32 + n_init - 1) * (32 * nBlocksV + n_init - 1)) >> > (aux1_d, width, height, img_pitch / sizeof(stbi_uc), aux1_pitch, n_init, nBlocksV, img_d);
...
int main(void) {
	int nk, *restrict ns, i, width, height, chn, f, nBlocksV, threadsH, nBlocksH;
